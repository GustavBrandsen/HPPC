...
public:
    ...
    std::string filename = "OG_trajectory.txt"; // name of the output file with trajectory
    ...
    for (size_t i = 0; i < sys.molecules.size(); i++)
        ...
        for (auto &j : sys.molecules[i].neighbours) // iterate over all neighbours of molecule i
            for (auto &atom1 : sys.molecules[i].atoms)
                for (auto &atom2 : sys.molecules[j].atoms)
                {                                                              // iterate over all pairs of atoms, similar as well as dissimilar
