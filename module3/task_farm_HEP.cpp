...
void master(int nworker, Data &ds)
{
    ...
    */
    ...
    for (int i = 0; i < nworker && tasks_sent < n_settings; i++)
    {
        MPI_Send(settings[tasks_sent].data(), 8, MPI_DOUBLE, i + 1, 0, MPI_COMM_WORLD);
        tasks_sent++;
    }
    ...
    while (tasks_received < n_settings)
    {
        ...
        MPI_Status status;
        MPI_Recv(&worker_accuracy, 1, MPI_DOUBLE, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &status);
        int worker_rank = status.MPI_SOURCE;
        accuracy[tasks_received] = worker_accuracy;
        tasks_received++;

        if (tasks_sent < n_settings)
        {
            MPI_Send(settings[tasks_sent].data(), 8, MPI_DOUBLE, worker_rank, 0, MPI_COMM_WORLD);
            tasks_sent++;
        }
        else
        {
            double termination_signal[8] = {-1, -1, -1, -1, -1, -1, -1, -1};
            MPI_Send(termination_signal, 8, MPI_DOUBLE, worker_rank, 0, MPI_COMM_WORLD);
        }
    }
    // for (long k = 0; k < n_settings; k++)
...
void worker(int rank, Data &ds)
{
    ...
    while (true)
    {
        // Receive a settings from the master
        MPI_Recv(setting.data(), 8, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);

        if (setting[0] == -1) // Termination signal
            break;

        double accuracy = task_function(setting, ds);
